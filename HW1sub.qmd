---
title: "Homework 1"
author: "[Adam Whiteside]{style='background-color: yellow;'}"
toc: true
...
I expect that the row_wise_scan would take shorter, because I would guess that going across the columns over and over again would take time.
...
The row wise time function took longer to run.
...
```{R}
M <- matrix(1:10000, nrow = 100, byrow = TRUE)
list(
    row_wise_time = time_scan(row_wise_scan, M),
    col_wise_time = time_scan(row_wise_scan, M)
)
...
```{R}
M <- matrix(1:(1000*1000), nrow = 1000, byrow = TRUE)
list(
    row_wise_time = time_scan(row_wise_scan, M),
    col_wise_time = time_scan(row_wise_scan, M)
)
...
```{R}
M <- matrix(1:(5000*5000), nrow = 5000, byrow = TRUE)
list(
    row_wise_time = time_scan(row_wise_scan, M),
    col_wise_time = time_scan(row_wise_scan, M)
)
...
I can conclude that the col_wise_time function is faster, shown in the three tests above, although a small difference, the col_wise_time had a shorter run time for all three.
---
